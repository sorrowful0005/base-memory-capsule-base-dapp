...
const source = await readFile(resolve(process.cwd(), "contracts/BaseMemoryCapsule.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: {
    "BaseMemoryCapsule.sol": { content: source },
  },
  settings: {
    outputSelection: {
      "*": {
...
const contract = output.contracts?.["BaseMemoryCapsule.sol"]?.BaseMemoryCapsule;
if (!contract) {
...
console.log(`Deploying BaseMemoryCapsule from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_MEMORY_CAPSULE_CONTRACT_ADDRESS to this contract address in Vercel.");
